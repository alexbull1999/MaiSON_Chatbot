    ...
    return {
        "elements": [
            ...
            },
            {
                ...
                "lat": 51.5077,
                "lon": -0.1281,
                "tags": {"public_transport": "station", "name": "Test Station"},
            },
            {
                "type": "node",
                "id": 5,
...
@pytest.mark.asyncio
async def test_get_area_insights_broad_area(
    ...
    try:
        ...
        ), "Result should be an AreaInsights instance"
        assert (
            insights.market_overview is not None
        ), "Market overview should not be None"
        assert isinstance(
            insights.area_profile.amenities_summary, dict
        ), "Amenities summary should be a dict"
        assert isinstance(
            insights.area_profile.transport_summary, dict
        ), "Transport summary should be a dict"
        assert isinstance(
            insights.area_profile.education, dict
        ), "Education should be a dict"

        # Specific value assertions
        assert (
            insights.market_overview.average_price == 350000.0
        ...
        assert (
            sum(amenities.values()) >= 1
        ), f"Expected at least 1 amenity, got {sum(amenities.values())}"
        ...
        if "schools" in education:
            ...
            # Current structure
            assert any(
                key in education for key in ["Unknown", "primary", "secondary"]
            ), "Should have school categories"
            category = next(iter(education))
            assert "count" in education[category], "Should have school count"
            assert (
                "average_distance" in education[category]
            ), "Should have average distance"
