...
def print_conversations(user_id, conversations, filter_desc=""):
    ...
    for conv in conversations.get("property_conversations", []):
        counterpart_flag = "COUNTERPART" if conv.get("is_counterpart", False) else "DIRECT"
        print(f"  - ID: {conv['id']}, Property: {conv['property_id']}, Role: {conv['role']}, Status: {counterpart_flag}")
...
def main():
    ...
    buyer_role_count = print_conversations(BUYER_ID, buyer_role_conversations, "(role=buyer)")
    ...
    seller_role_count = print_conversations(SELLER_ID, seller_role_conversations, "(role=seller)")
    ...
    buyer_status_count = print_conversations(BUYER_ID, buyer_status_conversations, "(status=active)")
    ...
    buyer_closed_count = print_conversations(BUYER_ID, buyer_closed_conversations, "(status=closed)")
    ...
    if buyer_role_count != 1:
        print("❌ FAILED: Buyer should see exactly 1 conversation with role=buyer filter")
        all_tests_passed = False
    
    if seller_role_count != 1:
        print("❌ FAILED: Seller should see exactly 1 conversation with role=seller filter")
        all_tests_passed = False
    
    if buyer_status_count != 1:
        print("❌ FAILED: Buyer should see exactly 1 conversation with status=active filter")
        all_tests_passed = False
